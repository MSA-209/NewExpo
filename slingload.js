...
export function SlingloadScreen({ navigation, route }) {
  ...
  return (
    ...
    <Text style={[styles.inspectorTitle]}>SLINGLOAD INSPECTOR</Text>
      ...
      <View style={{display: isPhone? 'flex' : 'none', position: 'absolute', zIndex: 30, left: 205, top: -80}}>
          ...
          <View style={{display: isPhone? 'none' : 'flex', position: 'absolute', zIndex: 30, top: -172, left: 445}}>
            <SlingloadDropdown style={{position: 'absolute'}}/>
    </View>
...
export function PlacardVideo({ navigation, route }) {
  ...
  <View style={{position: 'absolute', zIndex: 10, right: isPhone? 220 : 0, top: isPhone? 50 : 63}}>
            <SlingloadDropdown style={{position: 'absolute'}}/>
        </View>
  </View>
</ScrollView>  
...
export function PlacardSequence({ navigation, route }) {
  ...
  <View style={{position: 'absolute', zIndex: 10, right: isPhone? 270 : 0, top: isPhone? 125 : 63}}>
            <SlingloadDropdown style={{position: 'absolute'}}/>
        </View>
  </View>
</ScrollView>  
...
const SlingloadDropdown = () => {
  ...
  return (      
    <View style={{top: isPhone? 8 : -8, right: isPhone? 0 : -30}}>
      <View style={{zIndex: 9,  width: isPhone? 'auto' : 420}}>
      <TouchableOpacity onPress={handleHamburgerClick}>
      <View style={{backgroundColor: 'rgba(0, 0, 0, 0.7)', paddingTop: 8, paddingBottom: 8, 
        paddingRight: 12, paddingLeft: 12, borderRadius: 10, position: 'absolute', zIndex: 10, right: isPhone? -215 : 45, top: isPhone? 5 : 10}}>
                <FontAwesome name="bars" size={isPhone? 24 : 40} color='#E8E2D9'/>
            </View>
    </TouchableOpacity>
      </View>
      {menuVisible && (       
      <View style={{backgroundColor:'rgba(0, 0, 0, 0.8)', position: 'absolute', borderRadius: 0}}>
          <View style={[styles.menuSection, {top: isPhone? 'auto' : 0, backgroundColor: '#5d5e5e', height: isPhone? 50 : 'auto', marginBottom: isPhone? 5 : 5, borderRadius: 0}]}>
            <Text style={[styles.sectionMenuTitle, {fontSize: isPhone? 15 : 23, alignSelf: 'center', zIndex: 15}]}>SECTION MENU</Text>
        </View>
      <ScrollView style={{height: isPhone? 360 : 400, top: isPhone? 10 : 15}} showsVerticalScrollIndicator={true}>
      {titles.map((title, index) => (
        <TouchableOpacity key={index} onPress={() =>  {handleItemClick(title)}}>
        <View style={[styles.menuItemBox,  {marginLeft: isPhone? 10 : 20, marginBottom: isPhone? 5 : 10,  marginRight: isPhone? 10 : 20, backgroundColor: '#232425', paddingLeft: isPhone? 10 : 25, justifyContent: 'center'}]}>
    ...
    //   visible={menuVisible}
    //   onDismiss={closeMenu}
    //   anchor={<FontAwesome name="bars" onPress={openMenu} backgroundColor={"#FFFFFF"} />}
